...
export const getUserDashboard = asyncHandler(async (req, res) => {
  ...
  const [cart, activeOrders, userWithMembership, availableMemberships] = await Promise.all([
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { categories, cartCount, activeOrders, currentMembership, availableMemberships },
...
export const selectMembership = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, membership, 'Membership selected successfully'))
})
...
export const extendMembership = asyncHandler(async (req, res) => {
  ...
  if (!allowedDurations.includes(extensionValue)) {
    throw new ApiError(400, 'Extension must be 6 months, 1 year, or 2 years')
  }
...
export const getVendors = asyncHandler(async (req, res) => {
  ...
  const vendors = await User.find(filter).select('name email category profileImage')

  const vendorsWithProducts = await Promise.all(
    vendors.map(async (vendor) => {
      const products = await Product.find({ vendor: vendor._id, isActive: true }).select(
  ...
  res.status(200).json(new ApiResponse(200, vendorsWithProducts, 'Vendors fetched successfully'))
})

export const getGuests = asyncHandler(async (req, res) => {
  const guests = await Guest.find({ user: req.user._id }).sort({ createdAt: -1 })
  res.status(200).json(new ApiResponse(200, guests, 'Guest list fetched'))
})
