...
function UserManagement() {
	...
	const cancelMembership = async (userId) => {
		try {
			...
			setError(requestError?.response?.data?.message || 'Unable to cancel membership right now.')
		}
	}
		...
		<PageShell title="User And Vendor Management" links={[{ to: '/admin/memberships', label: 'Memberships' }]}>
			<section className="space-y-4">
				<div className="flex flex-wrap items-center gap-3">
					<label className="text-sm font-semibold text-slate-700">Role Filter</label>
					<select
						value={role}
