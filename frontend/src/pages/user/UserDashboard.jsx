...
function UserDashboard() {
	...
	const selectMembership = async (membershipNumber) => {
		...
		try {
			await apiClient.patch('/user/membership/select', { membershipNumber })
			setMessage('Membership selected successfully.')
			refreshDashboard()
		} catch (requestError) {
			setError(requestError?.response?.data?.message || 'Unable to select membership right now.')
		}
	}
	...
	const extendMembership = async () => {
		...
		try {
			await apiClient.patch('/user/membership/extend', { extensionMonths: Number(extensionMonths) })
			setMessage('Membership extended successfully.')
			setExtensionMonths(6)
			refreshDashboard()
		} catch (requestError) {
			setError(requestError?.response?.data?.message || 'Unable to extend membership right now.')
		}
	}
	...
	return (
		...
		>
				...
				<article className="rounded-md border border-slate-300 bg-white p-4">
					...
					{stats.currentMembership ? (
						<div className="mt-3 space-y-1 text-sm text-slate-700">
							...
							<p>
								<span className="font-semibold">Term:</span> {formatTerm(stats.currentMembership.durationMonths)}
							</p>
							<p>
								<span className="font-semibold">Price:</span> Rs {stats.currentMembership.price}
							</p>
							<p>
								<span className="font-semibold">Expiry:</span>{' '}
								{stats.currentMembership.endDate
									? new Date(stats.currentMembership.endDate).toLocaleDateString()
									: '-'}
							</p>
								...
								<div className="flex gap-2">
									<button
										...
										className={`flex-1 rounded-md px-2 py-2 text-xs font-semibold ${
											extensionMonths === 6 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'
										}`}
									>
										6 Months
									</button>
									<button
										...
										className={`flex-1 rounded-md px-2 py-2 text-xs font-semibold ${
											extensionMonths === 12 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'
										}`}
									>
										1 Year
									</button>
									<button
										type="button"
										onClick={() => setExtensionMonths(24)}
										className={`flex-1 rounded-md px-2 py-2 text-xs font-semibold ${
											extensionMonths === 24 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'
										}`}
									>
										2 Years
									</button>
						...
						<p className="mt-3 text-sm text-slate-500">No membership selected yet.</p>
					)}
				</article>

				<article className="rounded-md border border-slate-300 bg-white p-4">
					<h2 className="text-lg font-semibold text-slate-800">Available Memberships</h2>

					<div className="mt-3 space-y-3">
						{stats.availableMemberships.map((membership) => (
							<div key={membership._id} className="rounded-md border border-slate-200 bg-slate-50 p-3">
								<p className="text-sm font-semibold text-slate-800">{membership.name}</p>
								<p className="text-xs text-slate-600">No: {membership.membershipNumber}</p>
								<p className="text-xs text-slate-600">
									{formatTerm(membership.durationMonths)} | Rs {membership.price}
								</p>
								<button
									type="button"
									onClick={() => selectMembership(membership.membershipNumber)}
						...
						{stats.availableMemberships.length === 0 && (
							<p className="text-sm text-slate-500">No active memberships available.</p>
						)}
					</div>
				</article>
			</section>
			...
			{message && <p className="rounded-md bg-emerald-100 px-3 py-2 text-sm text-emerald-700">{message}</p>}

			<section className="rounded-md border border-slate-300 bg-white p-4">
				<h2 className="text-lg font-semibold text-slate-800">Available Categories</h2>
				<div className="mt-3 flex flex-wrap gap-2">
					{stats.categories.map((category) => (
						<Link
							key={category}
							to={`/user/vendors?category=${encodeURIComponent(category)}`}
					...
					{stats.categories.length === 0 && <p className="text-sm text-slate-500">No categories yet.</p>}
				</div>
			</section>
		</PageShell>
	)
