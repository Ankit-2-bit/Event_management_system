		...
		>
			<section className="space-y-4">
				<p className="text-sm text-slate-600">
					Filter: <span className="font-semibold">{selectedCategory || 'All Categories'}</span>
				</p>
				...
				<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
					{vendors.map((vendor) => (
						<article key={vendor._id} className="rounded-md border border-slate-300 bg-blue-600 p-4 text-white">
							{vendor.profileImage ? (
								<img
								...
								<div className="mb-3 flex h-32 w-full items-center justify-center rounded-md bg-blue-500">
									<span className="text-4xl font-bold text-blue-200">{vendor.name.charAt(0).toUpperCase()}</span>
								</div>
							)}
							<h2 className="text-lg font-semibold">{vendor.name}</h2>
							<p className="text-sm text-blue-100">{vendor.category}</p>
							<p className="mt-2 text-xs text-blue-100">{vendor.email}</p>
							...
							<div className="mt-3 space-y-2">
								{vendor.products?.slice(0, 4).map((product) => (
									...
									>
										...
										<div className="text-xs text-blue-100">Rs {product.price}</div>
										<button
											type="button"
											onClick={(e) => {
												e.stopPropagation()
								...
								{!vendor.products?.length && <p className="text-xs text-blue-100">No products available yet.</p>}
							</div>
						</article>
					))}
			...
			{selectedProduct && (
				<div className="fixed inset-0 flex items-center justify-center bg-black/50 p-4 z-50">
					<div className="max-h-96 w-full max-w-md overflow-y-auto rounded-lg bg-white p-6 shadow-lg">
						<button
							type="button"
							onClick={closeProductModal}
							className="absolute top-4 right-4 text-2xl font-bold text-slate-600 hover:text-slate-900"
						>
							✕
						</button>
						...
						<p className="mt-2 text-lg font-semibold text-green-600">Rs {selectedProduct.price}</p>
						<p className="mt-1 text-sm text-slate-600">Stock: {selectedProduct.stock}</p>
